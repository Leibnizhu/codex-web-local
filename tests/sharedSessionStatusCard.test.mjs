...
test('App injects SharedSessionStatusCard into the ThreadConversation scroll flow when snapshot exists', async () => {
  ...
  assert.match(app, /:live-approval-count="selectedThreadServerRequests.length"/)
  assert.match(app, /:persisted-approval-count="selectedThreadPersistedServerRequests.length"/)
